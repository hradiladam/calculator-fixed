// TESTS/playwright-tests/test-scripts/ui-behavior.test.ts
...
test.describe('UI Behavior & Layout', () => {
    ...
    // When recent history is too tall, it should be possible to scroll vertically
    test('recent history scrolls vertically when multiple long expressions are added', async () => {
        test.setTimeout(60_000);
        const longExpr = '1234567890+'.repeat(10) + '1=';
        for (let i = 0; i < 5; i++) await calculator.pressSequence(longExpr);

        const scrollable = await calculator.recentHistory.evaluate(el => el.scrollHeight > el.clientHeight);
...
// npx playwright test TESTS/playwright-tests/test-scripts/ui-behavior.test.ts --config=TESTS/playwright-tests/playwright.config.ts
// npx playwright test TESTS/playwright-tests/test-scripts/ui-behavior.test.ts --config=TESTS/playwright-tests/playwright.config.ts --project=Chromium